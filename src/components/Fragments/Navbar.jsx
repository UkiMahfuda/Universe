  ...
  return (
    <Fragment>
      ...
      >
        ...
        <div id="topNavbar" className="bg-transparent hidden text-white lg:flex justify-evenly px-14 pt-6 p-3 poppins-semibold">
          {NavbarList.map((list) => (
      ...
      <div
        id="bottomNavbar"
        className={`lg:hidden poppins-semibold transition-all duration-300 bottom-0 left-0 right-0 z-50 text-white fixed flex justify-evenly bg-secondary py-1    
    ${isScrolled ? "bg-opacity-50 backdrop-blur-sm" : ""}`}
      >
        {NavbarList.map((list) => (
          <BtnNavbar key={list.name} link={list.link}>
